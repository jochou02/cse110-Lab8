describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let prodItemElem = await page.$('product-item');
    // Grab the shadowRoot of that element (it's a property), then query a button from that shadowRoot.
    let shadowRoot = await prodItemElem.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let prodItemElems = await page.$$('product-item');
    for(let i = 1; i < prodItemElems.length; i++) {
    // get the shadowRoot and query select the button inside, and click on it.
      let shadowRoot = await prodItemElems[i].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      await button.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    let prodItemElems = await page.$$('product-item');
    for (let i = 0; i < prodItemElems.length; i++) {
      let shadowRoot = await prodItemElems[i].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      let innerText = await button.getProperty('innerText');
      let textValue = await innerText.jsonValue();
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cartCount = await page.evaluate(() => localStorage.getItem('cart'));
